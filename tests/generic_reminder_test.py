...
def test_check_reminder(patch_datetime_morning):
    ...
    assert FAKE_TIME_REMINDER.date() == patch_datetime_morning.next_generic.date.date()
...
def patch_datetime_last_minute(monkeypatch):
    ...
    monkeypatch.setattr(next_event_tracker, "NextEvent", myNextEvent)
    return reminder

def test_check_last_minute(patch_datetime_last_minute):
    assert datetime.datetime.now() == FAKE_TIME_LAST_MINUTE
    assert FAKE_TIME_LAST_MINUTE.date() == patch_datetime_last_minute.next_generic.date.date()
